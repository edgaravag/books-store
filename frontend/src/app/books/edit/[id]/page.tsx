...
const CreateBook = () => {
  const { register, handleSubmit, reset, formState: { errors, isDirty, isValid, touchedFields } } = useForm<FormData>();
  const router = useRouter()
  const { id } = useParams()

  useEffect(() => {
    axios.get(`${process.env.NEXT_PUBLIC_MY_API_URL}/${id}`)
    .then((res) => {
      const { title, author, publishYear, price, description, language } = res.data;
      reset({
        title,
        author,
        publishYear,
  ...
  const handleEditBook = (data: FormData) => {
    axios.put(`${process.env.NEXT_PUBLIC_MY_API_URL}/${id}`, data)
    .catch((error) => {
      console.error(error);
    });
  };

  const onEditHandler: SubmitHandler<FormData> = async (data: FormData) => {
    try {
      ...
      <div className="flex flex-col gap-7 animate-fade">
        ...
        <div className="w-full md:w-[40rem] mx-auto pt-8 pb-6 px-6 h-auto border-4 border-dotted border-slate-500 rounded-2xl bg-white shadow-myInner">
          ...
          >
            ...
            <div className="flex flex-col gap-2">
              <label className="text-xl font-semibold font-money">Description</label>
              <input
